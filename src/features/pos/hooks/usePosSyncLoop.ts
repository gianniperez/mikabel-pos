...
import { writeBatch, doc, collection, increment, getDoc } from "firebase/firestore";
import { toast } from "sonner";

export const usePosSyncLoop = () => {
  const syncQueue = usePosStore((state) => state.syncQueue);
  const removeTicketFromQueue = usePosStore((state) => state.removeTicketFromQueue);
    ...
    const processQueue = async () => {
      ...
      for (const ticket of syncQueue) {
        try {
          ...
          if (!saleSnap.exists()) {
            ...
            if (ticket.splitPayments) {
              ...
              if (ticket.splitPayments.cash > 0) {
                updateData.totalCashSales = increment(ticket.splitPayments.cash);
              }
              if (ticket.splitPayments.transfer > 0) {
                updateData.totalTransferSales = increment(ticket.splitPayments.transfer);
              }
              if (ticket.splitPayments.debt && ticket.splitPayments.debt > 0) {
                updateData.totalDebtSales = increment(ticket.splitPayments.debt);
            ...
            await batch.commit();
            console.log(`[SyncLoop] Ticket ${ticket.id} sincronizado exitosamente.`);
          } else {
            console.warn(`[SyncLoop] Ticket ${ticket.id} ya existía en Firebase. Limpiando cola local.`);
          }
          ...
          console.error(`[SyncLoop] Falló la sincronización del ticket ${ticket.id}. Se reintentará luego.`, error);
          break; 
        }
      }
